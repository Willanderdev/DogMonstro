...
def finalizar_pedido(request, room_name='pedir'):
    if request.method == "GET":
        ...
        if len(request.session['carrinho']) > 0:
            ...
            def lambda_func_troco(x): return int(x['troco_para'])-total if not x['troco_para'] == '' else ""

            def lambda_func_pagamento(x): return 'Cartão' if x['meio_pagamento'] == '2' else 'Dinheiro'
